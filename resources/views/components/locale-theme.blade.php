...
<div class="flex items-center gap-2"
    ...
    x-data="{
        ...
        lightTitle: @js(__('app.theme.light')),
        darkTitle: @js(__('app.theme.dark')),
        toggleTheme() {
            this.dark = !this.dark;
    ...
    <div class="locale-switcher flex rounded-lg overflow-hidden border border-[var(--fc-border)] text-xs font-semibold">
        ...
        <a href="{{ route('locale.switch', $code) }}"
           class="px-2.5 py-1.5 transition {{ app()->getLocale() === $code ? 'bg-[var(--npuu-accent)] text-white' : 'bg-[var(--fc-card)] text-[var(--fc-text-muted)] hover:bg-[var(--fc-hover)]' }}">
            {{ $label }}
        </a>
        @endforeach
    </div>
    ...
    <button type="button" @click="toggleTheme()"
        class="p-2 rounded-lg border border-[var(--fc-border)] bg-[var(--fc-card)] text-[var(--fc-text-muted)] hover:text-[var(--npuu-accent)] transition"

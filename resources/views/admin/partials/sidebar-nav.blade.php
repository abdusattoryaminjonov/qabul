<nav class="sidebar-nav" x-data="{ formsOpen: {{ request()->routeIs('admin.forms.*') ? 'true' : 'false' }} }">
    ...
    <div class="sidebar-group">
        <button type="button" class="sidebar-link sidebar-link-group w-full {{ request()->routeIs('admin.forms.*') ? 'active' : '' }}" @click="formsOpen = !formsOpen">
            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            <span class="flex-1 text-left">{{ __('app.nav.forms') }}</span>
            <svg class="w-4 h-4 shrink-0 transition-transform" :class="formsOpen && 'rotate-180'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
        </button>
        <div x-show="formsOpen" class="sidebar-sublinks">
            <a href="{{ route('admin.forms.index') }}" class="sidebar-sublink {{ request()->routeIs('admin.forms.index') || request()->routeIs('admin.forms.edit') ? 'active' : '' }}" @click="sidebarOpen = false">
                <span class="sidebar-sublink-dot" aria-hidden="true"></span>
                {{ __('app.nav.all_forms') }}
            </a>
            <a href="{{ route('admin.forms.create') }}" class="sidebar-sublink {{ request()->routeIs('admin.forms.create') ? 'active' : '' }}" @click="sidebarOpen = false">
                <span class="sidebar-sublink-dot" aria-hidden="true"></span>
                {{ __('app.nav.new_form') }}
            </a>
        </div>

...
<body class="min-h-screen @auth admin-bg @else auth-page @endauth">
    ...
    <div class="flex min-h-screen" x-data="{ sidebarOpen: false }" @keydown.escape.window="sidebarOpen = false">
        ...
        <aside class="sidebar sidebar-drawer flex flex-col w-[17.5rem] shrink-0 fixed inset-y-0 left-0 z-50"
            :class="sidebarOpen ? 'sidebar-drawer-open' : ''">
            <div class="sidebar-brand p-5 flex items-center justify-between gap-3">
                <a href="{{ route('admin.dashboard') }}" class="min-w-0" @click="sidebarOpen = false">
                    <x-brand-logo variant="admin-panel" />
                </a>
            ...
            <div class="flex-1 px-3 pb-6 overflow-y-auto">
                @include('admin.partials.sidebar-nav')
            </div>
        </aside>

        <div class="flex-1 lg:ml-[17.5rem] min-w-0 flex flex-col w-full">
            <header class="admin-header admin-header-fixed">
                    ...
                    <div class="admin-header-right">
                        ...
                        <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                            <button type="button" @click="open = !open"
                                class="header-notify {{ request()->routeIs('admin.notifications.*') ? 'header-notify-active' : '' }}"
                            ...
                            <div x-show="open" x-transition
                                ...
                                <div class="px-4 py-2.5 border-b border-[var(--fc-border)] flex items-center justify-between gap-2">
                                    ...
                                    <form action="{{ route('admin.notifications.read-all') }}" method="POST">@csrf
                                        <button type="submit" class="text-xs text-[var(--admin-navy)] font-medium hover:underline">{{ __('app.notifications.mark_all_read') }}</button>
                                    </form>
                                    @endif
                                </div>
                                ...
                                <div class="border-t border-[var(--fc-border)] px-2 py-1.5">
                                    <a href="{{ route('admin.notifications.index') }}" class="header-dropdown-item justify-center text-[var(--admin-navy)] font-medium">
                        ...
                        <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                            <button type="button" @click="open = !open"
                                class="header-profile {{ request()->routeIs('admin.profile.*') ? 'header-profile-active' : '' }}">
                                <div class="header-profile-avatar">
                                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                                </div>
                                <div class="hidden md:block text-left min-w-0">
                                    <div class="header-profile-name">{{ auth()->user()->name }}</div>
                                    <div class="header-profile-role">{{ auth()->user()->roleLabel() }}</div>
                                </div>
                            ...
                            <div x-show="open" x-transition
                                ...
                                <div class="px-4 py-2.5 border-b border-[var(--fc-border)]">
                                    ...
                                    <div class="text-xs font-medium text-[var(--admin-navy)] truncate mt-0.5">{{ auth()->user()->roleLabel() }}</div>
                                </div>
            ...
            <main class="flex-1 min-w-0 admin-main-offset">
                @yield('content')
            </main>
        </div>
    </div>

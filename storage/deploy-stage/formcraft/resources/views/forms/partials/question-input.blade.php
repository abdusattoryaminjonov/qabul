...
@switch($question->type)
    ...
    @case('dropdown')
        <select name="answers[{{ $question->id }}]" class="input">
            ...
            <option value="{{ $option->text }}" {{ old('answers.'.$question->id) === $option->text ? 'selected' : '' }}>{{ $option->text }}</option>
            @endforeach
        </select>
        @break
    @case('linear_scale')
        <div class="mt-1">
            <div class="flex justify-between text-xs font-medium text-fc-muted mb-3 px-0.5">
                <span>{{ $question->settings['min_label'] ?? $min }}</span>
                <span>{{ $question->settings['max_label'] ?? $max }}</span>
            </div>

    ...
    <div x-show="tab === 'questions'" class="builder-canvas py-8">
        ...
        <div class="form-card overflow-hidden">
            ...
            <div class="p-6 lg:p-8">
                <input type="text" x-model="formTitle" placeholder="{{ __('app.builder.form_title') }}"
                ...
                <textarea x-model="formDescription" placeholder="{{ __('app.builder.form_description') }}" rows="2"
        ...
        <div id="questions-list" class="space-y-4">
            ...
            <template x-for="(question, qIndex) in questions" :key="question.uid">
                <div class="question-builder-card question-card" :data-uid="question.uid">
                    <div class="flex items-start gap-3 p-4 border-b border-slate-100 bg-slate-50/80 rounded-t-xl">
                        <div class="drag-handle cursor-grab active:cursor-grabbing text-slate-400 hover:text-violet-500 pt-1 transition" title="Surish">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M7 2a2 2 0 11.001 4.001A2 2 0 017 2zm0 6a2 2 0 11.001 4.001A2 2 0 017 8zm0 6a2 2 0 11.001 4.001A2 2 0 017 14zm6-8a2 2 0 11-.001-4.001A2 2 0 0113 6zm0 2a2 2 0 11.001 4.001A2 2 0 0113 8zm0 6a2 2 0 11.001 4.001A2 2 0 0113 14z"/></svg>
                        </div>
                        <div class="flex-1">
                            <label class="block text-xs font-semibold text-fc-muted uppercase tracking-wide mb-1">{{ __('app.builder.question_text_label') }} *</label>
                            <input type="text" x-model="question.title" placeholder="{{ __('app.builder.question_text') }}"
                                class="w-full font-semibold border-0 border-b-2 border-violet-200 focus:border-violet-400 outline-none bg-transparent pb-1 text-slate-900">
                        </div>
                        <select x-model="question.type" @change="onTypeChange(question)" class="input text-xs py-1.5 px-2 w-auto shrink-0">
                    ...
                    <div class="p-4">
                        <input type="text" x-model="question.description" placeholder="{{ __('app.builder.help_text') }}"
                            ...
                            <div class="space-y-2 ml-2">
                                <template x-for="(opt, oIndex) in question.options" :key="opt.uid">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <input type="text" x-model="opt.text" :placeholder="i18n.option"
        ...
        <div class="card border-2 border-dashed border-violet-200 bg-violet-50/30 p-6">
            ...
            <div class="flex flex-wrap justify-center gap-2">
                @foreach(\App\Models\FormQuestion::typeLabels() as $key => $label)
                <button @click="addQuestion('{{ $key }}')" class="text-xs font-medium bg-white border border-slate-200 rounded-full px-3.5 py-2 hover:bg-violet-600 hover:text-white hover:border-violet-600 transition shadow-sm">
                    {{ $label }}
                </button>
                @endforeach
            </div>
    ...
    <div x-show="tab === 'settings'" class="p-6 lg:p-10 max-w-2xl mx-auto">
        <form method="POST" action="{{ route('admin.forms.update', $form) }}" class="card p-6 lg:p-8 space-y-6">
            ...
            <div class="bg-violet-500/10 rounded-xl p-4 border border-violet-500/20">
                <label class="block text-sm font-semibold text-fc mb-2">{{ __('app.builder.share_link') }}</label>
                <div class="flex gap-2">
                    <input type="text" readonly value="{{ $form->publicUrl() }}" id="share-link" class="input flex-1 text-sm">
                    <button type="button" onclick="navigator.clipboard.writeText(document.getElementById('share-link').value); alert(@json(__('app.common.copied')))" class="btn btn-primary shrink-0">{{ __('app.common.copy') }}</button>
                </div>
            </div>
    ...
    return {
        ...
        onTypeChange(question) {
            if (this.hasOptions(question.type) && (!question.options || question.options.length === 0)) {
                question.options = [
                    { uid: this.newUid(), text: 'Variant 1', is_correct: false, score_value: 0, order: 0 },
                    { uid: this.newUid(), text: 'Variant 2', is_correct: false, score_value: 0, order: 1 },
                ];
            }
        ...
        addOption(question) {
            question.options.push({ uid: this.newUid(), text: '', is_correct: false, score_value: 0, order: question.options.length });
        },
